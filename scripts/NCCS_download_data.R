...
getcorefile <- function(coreyear, coretype) {
  ...
  #create URL for download based on year of file (I had to edit this, links were old)
  if(coretype =="co"){
    URL <- paste("http://nccs-data.urban.org/dl.php?f=core/", as.character(coreyear), "/coreco.core", as.character(coreyear),as.character(coretype), ".csv", sep ="") 
  }
...
pc2016 <- getcorefile(2016, "pc")

#filter out larimer, weld, morgan, denver, pueblo, alamosa counties
pc2016 %>% mutate(FIPS = stringr::str_sub(FIPS, start = 1, end = -3)) %>% 
  filter(FIPS %in% c(08069, 08123, 08087, 08031, 08101, 08003)) %>% distinct(LATITUDE, LONGITUDE)
# 1537 public charities

pc_2017_full <- vroom::vroom("../../Downloads/coreco.core2017pc_full990.csv")
pc_2016_full <- vroom::vroom("data/coreco.core2016pc_full990.csv")


#which onces are in full and not original
pc_2016_full %>% filter(!(NAME %in% pc2016$NAME)) #they are all in here...

#most up to date
co2018 <- vroom::vroom("data/coreco.core2018co.csv") %>% mutate(FIPS = stringr::str_sub(FIPS, start = 1, end = -3)) %>% 
  filter(FIPS %in% c(08069, 08123, 08087, 08031, 08101, 08003)) %>%
  mutate(full_address = paste(ADDRESS, CITY, STATE, ZIP, sep = ","))

pc2018 <- vroom::vroom("data/coreco.core2018pc.csv") %>%  
  filter(FIPS %in% c(08069, 08123, 08087, 08031, 08101, 08003)) %>% 
  mutate(full_address = paste(ADDRESS, CITY, STATE, ZIP, sep = ","))


#recommeneded cleaning steps

## filter out out of scope orgs OUTNCSS != "OUT"
pc2018 <- pc2018 %>% filter(OUTNCCS != "OUT") #just one out of scope removed

#test out geocoding (coordinates seem rather inaccurate)

#takes 40 mins....
pc2018 %>% distinct(full_address)
pc2018_geo <- geocode(pc2018, address = full_address)

write.csv(pc2018_geo, "pc2018_coords.csv")

# there are 132 of 2316 addresses are duplicated for pc
# 46 of 719 duplicated addresses for other charities (co)
